...
function Env(t, e) {
    ...
    return new class {
        ...
        post(t, e = () => {
            ...
            if (t.body && t.headers && !t.headers["Content-Type"] && (t.headers["Content-Type"] = "application/x-www-form-urlencoded"), t.headers && (t.headers["User-Agent"] = this.ua, delete t.headers["Content-Length"]), this.isSurge() || this.isLoon())
...
function TOTP(token) {
    function t(e, a, d) {
        var g = 0, c = [], b = 0, f, k, l, h, m, w, n, y, p = false, q = [], t = [], v, u = false;
        d = d || {};
        f = d.encoding || "UTF8";
        v = d.numRounds || 1;
        l = z(a, f);
        ...
        if ("SHA-1" === e)
            m = 512, w = A, n = H, h = 160, y = function (a) {
                return a.slice();
            };
        else
    ...
    function C(e, a, d) {
        ...
        var c, b;
        for (c = 0; c < a; c += 1)
            b = e[c >>> 2] >>> 8 * (3 - c % 4), g += "0123456789abcdef".charAt(b >>> 4 & 15) + "0123456789abcdef".charAt(b & 15);
        return d.outputUpper ? g.toUpperCase() : g;
    }
    function D(e, a, d) {
        var g = "", c = a / 8, b, f, k;
        for (b = 0; b < c; b += 3)
            for (f = b + 1 < c ? e[b + 1 >>> 2] : 0, k = b + 2 < c ? e[b + 2 >>> 2] : 0, k = (e[b >>> 2] >>> 8 * (3 - b % 4) & 255) << 16 | (f >>> 8 * (3 - (b + 1) % 4) & 255) << 8 | k >>> 8 * (3 - (b + 2) % 4) & 255, f = 0; 4 > f; f += 1)
    ...
    function E(e, a) {
        var d = "", g = a / 8, c, b;
        for (c = 0; c < g; c += 1)
            b = e[c >>> 2] >>> 8 * (3 - c % 4) & 255, d += String.fromCharCode(b);
        return d;
    }
    function F(e, a) {
    ...
    function A(e, a) {
        ...
        k = a[4];
        for (h = 0; 80 > h; h += 1)
            d[h] = 16 > h ? e[h] : p(d[h - 3] ^ d[h - 8] ^ d[h - 14] ^ d[h - 16], 1), l = 20 > h ? u(p(g, 5), c & b ^ ~c & f, k, 1518500249, d[h]) : 40 > h ? u(p(g, 5), c ^ b ^ f, k, 1859775393, d[h]) : 60 > h ? u(p(g, 5), c & b ^ c & f ^ b & f, k, 2400959708, d[h]) : u(p(g, 5), c ^ b ^ f, k, 3395469782, d[h]), k = f, f = b, b = p(c, 30), c = g, g = l;
        a[0] = q(g, a[0]);
        a[1] = q(c, a[1]);
        a[2] = q(b, a[2]);
        a[3] = q(f, a[3]);
